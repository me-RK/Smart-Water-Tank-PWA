 ...
 * - Connection status with ESP32 device
 * - System operation mode (Auto/Manual)
 * - Motor status and configuration
 * - Runtime and last update information
 * - Auto mode triggers and reasons
 * 
 * Features:
 * - Responsive design with compact layout
 * - Real-time status updates
 * - Color-coded status indicators
 * - Support for both legacy and v3.0 motor configurations
 * - Warning displays for critical states
 */
interface StatusCardProps {
  connected: boolean;                    // ESP32 connection status
  lastUpdated: string;                   // Last data update timestamp
  runtime: number;                       // System runtime in seconds
  motorStatus: 'ON' | 'OFF';            // Legacy motor status for backward compatibility
  motor1Status?: 'ON' | 'OFF';          // v3.0 Motor 1 current status
  motor2Status?: 'ON' | 'OFF';          // v3.0 Motor 2 current status
  motor1Enabled?: boolean;               // v3.0 Motor 1 enabled/disabled state
  motor2Enabled?: boolean;               // v3.0 Motor 2 enabled/disabled state
  motorConfig?: MotorConfiguration;      // v3.0 Motor configuration type
  mode: 'auto' | 'manual';              // System operation mode
  autoModeReasons: string[];            // Legacy auto mode reasons array
...
export const StatusCard: React.FC<StatusCardProps> = ({
  ...
  /**
   * Formats timestamp into localized time string
    ...
    `}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        ...
        <div className="flex items-center space-x-2">
          {connected ? (
            <Wifi className="w-4 h-4 text-green-500 transition-all duration-300 hover:scale-110 hover:rotate-12" />
          ) : (
            <WifiOff className="w-4 h-4 text-red-500 transition-all duration-300 hover:scale-110 hover:rotate-12" />
          )}
        ...
        {/* System Mode */}
        <div className="flex items-center space-x-2">
          <Settings className="w-4 h-4 text-indigo-500 transition-all duration-300 hover:scale-110 hover:rotate-12" />
          <div className="min-w-0">
            <p className="text-xs text-gray-500 dark:text-gray-400 truncate">
              Mode
            </p>
        ...
        {/* Transmitter Status */}
        <div className="flex items-center space-x-2">
          <Clock className={`w-4 h-4 transition-all duration-300 hover:scale-110 hover:rotate-12 ${runtime > 600 ? 'text-red-500' : runtime > 300 ? 'text-yellow-500' : 'text-green-500'}`} />
          <div className="min-w-0">
            <p className="text-xs text-gray-500 dark:text-gray-400 truncate">
              Last Data
            </p>
      ...
      {motor1Status !== undefined && motor2Status !== undefined ? (
        <div className="mb-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2">
              <Zap className="w-4 h-4 text-yellow-500 transition-all duration-300 hover:scale-110 hover:rotate-12" />
            ...
            {motorConfig && (
              <span className="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
                {getMotorConfigurationLabel(motorConfig)}
              </span>
            )}
          </div>
